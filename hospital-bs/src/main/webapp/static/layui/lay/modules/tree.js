...
//  ,leaf: '&#xe621;' //叶节点
  ...
  Tree.prototype.tree = function(elem, children){
    ...
    llwUrl = elem.attr("llw-tree-url");
      var llwKey = elem.attr("llw-tree-id") || "id",
          llwPid = elem.attr("llw-tree-pid") || "pid",
          llwLabel = elem.attr("llw-tree-label") || "name";
    ...
    if (llwUrl){
    	layui.util.ajax({
    		...
    		success : function(res){
    			if(res.respData) {
    				//重新组装数据
    				var TreeWapper = new layui.util.TreeWapper(res.respData, llwKey, llwPid);
    				that.createTree(elem, TreeWapper.toTree());
    			}
    		}
    	})
    }
    else{
    	that.createTree(elem, children);
    }
  };
  
  Tree.prototype.createTree = function(elem, children){
  	var that = this, options = that.options,
  ...
  $("ul[llw-tree-url]").each(function(){
	  ...
	  layui.tree({elem: this,click: function(node){
			 if (clickEvent){
