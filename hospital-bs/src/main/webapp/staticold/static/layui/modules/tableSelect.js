layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    tableSelect.prototype.getShowHtml = function(opt, tableName,t,l){
        var tableBox = '<div class="tableSelect layui-anim layui-anim-upbit" style="left:'+l+';top:'+t+';border: 1px solid #d2d2d2;background-color: #fff;box-shadow: 0 2px 4px rgba(0,0,0,.12);padding:10px 10px 0 10px;position: absolute;z-index:66666666;margin: 5px 0;border-radius: 2px;max-width:620px;">';
    ...
    tableSelect.prototype.getContentShowHtml = function(opt, tableName,t,l){
        var tableBox = '<div class="tableSelect layui-anim layui-anim-upbit" style="left:'+l+';top:'+t+';border: 1px solid #d2d2d2;background-color: #fff;box-shadow: 0 2px 4px rgba(0,0,0,.12);padding:10px 10px 0 10px;position: absolute;z-index:66666666;margin: 5px 0;border-radius: 2px;max-width:635px;">';
        tableBox += '<div class="tableSelectBar layui-form" style=""  lay-filter="tableSelectFilter">';
        tableBox += opt.content;
        tableBox += '</div>';
        tableBox += '<table id="'+tableName+'" lay-filter="'+tableName+'"></table>';
        tableBox += '</div>';
        return tableBox;
    ...
    tableSelect.prototype.render = function (opt) {
        ...
        opt.table.width = opt.table.width || 615;
        ...
        elem.off('click').on('click', function(e) {
            ...
            var tableBox = null;
            if (opt.content){
                tableBox = that.getContentShowHtml(opt, tableName, t, l);
            }
            else{
                tableBox = that.getShowHtml(opt, tableName, t, l);
            }

            that.target = tableBox = $(tableBox);
            $('body').append(tableBox);
            form.render("select", "tableSelectFilter");
            ...
            overWidth && tableBox.css({'left':'auto','right':($(window).width()-elem.offset().left-elem.width()-20)+'px'})
            // if (overWidth){
            //     tableBox.width($(window).width()-elem.offset().left-20);
            // }
            ...
            $(document).mouseup(function(e){
                var targetObj = $(opt.elem);
                var userSet_con = $('.tableSelect');
                if((!userSet_con.is(e.target) || !targetObj.is(e.target))
