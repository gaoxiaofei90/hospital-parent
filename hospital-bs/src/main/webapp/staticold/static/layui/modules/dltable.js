layui.define(['laytpl', 'laypage', 'layer', 'form','util'], function(exports){
    ...
    var $ = layui.$
        ...
        ,ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body', ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOOL = '.layui-table-tool', ELEM_PAGE = '.layui-table-page', ELEM_SORT = '.layui-table-sort', ELEM_EDIT = 'layui-table-edit', ELEM_HOVER = 'layui-table-hover',ELEM_CLICK = 'layui-table-click'
        ...
        ,TPL_BODY = ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
            ...
            ,'</table>'].join('')
        //主模板
        ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}{{# } }}">'
    ...
    Class.prototype.resetData=function(list) {
        ...
        if(list==null||list.length==undefined || list.length<=0)return datas;
        //设置默认参数
    ...
    Class.prototype.treeConvertShowName=function (o) {
        ...
        var temhtml='<div style="float: left;height: 28px;line-height: 28px;margin-right:5px;padding-left: '+
            function () {
                if(isTreeNode){
                    return '5px'
                }else{
            ...
            +function () {//图标或占位符
                var temTreeHtml='';
                if(options.checkbox)temTreeHtml='<input type="checkbox" name="checkList[]" lay-skin="primary"> ';
                return temTreeHtml;
            }()+'</div>';
        return temhtml;
    };
    ...
    Class.prototype.events = function(){
        ...
        that.elem.on('click', '.layui-form-checkbox', function(){
	        ...
	        var fa = function(e) {
	            if(e.children&&e.children.length>0){
	                ...
	                for (var i = 0; i < temList.length; i++) {
	                    ...
	                    if(checked){//打开状态的，关闭
	                        var temo=that.layBody.find('tr[data-index="'+ n[table.config.indexName] +'"]');
                            $("input[type=checkbox]", temo).each(function(i, item){
...
//      });
        ...
        //单击事件
        that.layBody.on('click', 'tr', function(){ //单击行
            var othis = $(this)
                ,index = othis.index();
            var rows=that.layBody.find('tr:not(:eq('+index+'))');
            rows.removeClass(ELEM_CLICK);
            var row=that.layBody.find('tr:eq('+ index +')');
            if(row.hasClass(ELEM_CLICK)){
                row.removeClass(ELEM_CLICK);
            }else{
                row.addClass(ELEM_CLICK);
            }
        ...
        var commonMember = function(sets){
            var othis = $(this)
                ,index = othis.parents('tr').eq(0).data('index')
                ,tr = that.layBody.find('tr[data-index="'+ index +'"]')
                ,data = table.cache[that.key].data.list[index];
...
//          }
    ...
    //自动完成渲染
    table.init();
//  layui.link('../../static/layui/modules/treeGrid.css');//引入css
