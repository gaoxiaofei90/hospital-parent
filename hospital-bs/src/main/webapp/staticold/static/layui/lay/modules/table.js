  ...
  Class.prototype.pullData = function(curr){
    ...
    if(options.url){ //Ajax请求
      ...
      if(typeof options.url === 'function'){
          finalUrl = options.url.call(this);
      }
        util.ajax({
        ...
        ,success: function(res){
          ...
          //设置code和msg字段
          res["code"] = (res["respCode"]||res["respCode"]==0)?res["respCode"]:res["code"];
          res["msg"] = res["respMsg"]?res["respMsg"]:res["msg"];
          res["data"] = (res["respData"] && res["respData"].list)?res["respData"].list:res["data"];
          res["count"] = (res["respData"] && res["respData"].total)?res["respData"].total:res["count"];
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      if (mainTbody.length == 0){
          mainTbody = $("<table cellspacing=\"0\" cellpadding=\"0\" border=\"0\" class=\"layui-table\"><tbody></tbody></table>");
          that.layMain.append(mainTbody);
          mainTbody = mainTbody.find('tbody');
      }
     ...
     if (!isShowOrg){
         for (var i=0;i<options.cols.length;i++){
             for (var j=0;j<options.cols[i].length;j++){
                 if (options.cols[i][j] && options.cols[i][j]["llwOrg"]){
                     options.cols[i].splice(j, 1);
                     break;
                 }
             }
         }
