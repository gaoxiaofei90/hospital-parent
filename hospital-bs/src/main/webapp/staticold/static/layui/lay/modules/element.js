	...
	Element.prototype.init = function(type, filter) {
		var that = this,
			...
			items = {
				...
				permission: function() {
					//增加权限的控制
					var permissionArr = parent.inst_permissionArr || parent.parent.inst_permissionArr ||
						parent.parent.parent.inst_permissionArr;

					if(permissionArr) {
						layui.$("input[llw-permission],a[llw-permission],button[llw-permission]").each(function() {
							var jobj = layui.$(this);
							if(permissionArr.indexOf(jobj.attr("llw-permission")) != -1) {
								jobj.show();
							} else {
								jobj.remove();
							}
				...
				dict: function() {
						...
						if(dictMap) {
							layui.$("[llw-dict-code]").each(function() {
								var tagName = this.tagName;
								var curObj = layui.$(this);
								var dictCode = curObj.attr("llw-dict-code");
								var dictValue = curObj.attr("llw-dict-value");
								if(dictValue){
									dictValue = dictValue.toString();
								}
								var dictArr = dictMap[dictCode];
								if("SELECT" != tagName.toUpperCase() && dictArr) {
									var showText = "";
									for(var i = 0, len = dictArr.length; i < len; i++) {
										if(dictArr[i].fieldValue == dictValue) {
											showText = dictArr[i].valueComment;
											break;
										}
									}
									curObj.text(showText);
								}
							});
						}
					},
                getName: function() {
                    //如果有字典，则显示字典的列表
                   // var getName = parent.getName || parent.parent.getName || parent.parent.parent.getName;
                    //if(getName) {
                        layui.$("[llw-getName-url]").each(function() {
                            var tagName = this.tagName;
                            var curObj = layui.$(this);
                            var url = curObj.attr("llw-getName-url");
                            var key = curObj.attr("llw-getName-key");
                            var pk = curObj.attr("llw-getName-pk");
                            var value=pk;
                            layui.$.ajax({
                                url:url,
                                data:{pk:pk},
