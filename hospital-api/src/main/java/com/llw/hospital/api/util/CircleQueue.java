...
public class CircleQueue<T> {
     ...
     * @return 取 循环队列里的值（先进的index=0）
     */
    public List<T> getQueue(Filter filter) {
        final Object[] elementDataSort = new Object[capacity];
        final Object[] elementDataCopy = elementData.clone();
        if (isEmpty()) {
        } else if (isFull()) {
        ...
        List<T> elementDataFilter = new ArrayList<T>();

        for(Object o:elementDataSort){
            if(null != o){
                if(filter.doFilter(o)){
                    elementDataFilter.add((T)o);
                }
            }
        }

        return elementDataFilter;
    }

    /**
     * @return 取 循环队列里的值（先进的index=0）
     */
    public List<T> getQueueReverse(Filter filter) {
        final Object[] elementDataSort = new Object[capacity];
        final Object[] elementDataCopy = elementData.clone();
        if (isEmpty()) {
        } else if (isFull()) {
        ...
        List<Object> list = Arrays.asList(elementDataSort);
    ...
    public static void main(final String[] args) {
        final CircleQueue<Object> queue = new CircleQueue<Object>(10);
        for (int i = 0; i < 2000000000; i++) {
            queue.add(new byte[1000]);
        }
